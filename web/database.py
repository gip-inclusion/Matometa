...
class ConversationStore:
    ...
    def get_conversation(self, conv_id: str, include_messages: bool = True, user_id: Optional[str] = None) -> Optional[Conversation]:
        ...
        with get_db() as conn:
            if user_id:
                row = conn.execute(
                    """SELECT c.*, p.pinned_at AS p_pinned_at, p.label AS p_label
                       ...
                       WHERE c.id = %s AND c.user_id = %s""", (conv_id, user_id)
                ).fetchone()
            else:
                row = conn.execute(
                    """SELECT c.*, p.pinned_at AS p_pinned_at, p.label AS p_label
            ...
            report_row = conn.execute(
                "SELECT * FROM reports WHERE conversation_id = %s", (conv_id,)
            ).fetchone()
    ...
    def unpin_item(self, item_type: str, item_id: str) -> bool:
        ...
        with get_db() as conn:
            cursor = conn.execute(
                "DELETE FROM pinned_items WHERE item_type = %s AND item_id = %s",
    ...
    def list_pinned_items(self, item_type: Optional[str] = None) -> list[PinnedItem]:
        ...
        with get_db() as conn:
            if item_type:
                rows = conn.execute(
                    "SELECT * FROM pinned_items WHERE item_type = %s ORDER BY pinned_at",
                    (item_type,)
                ).fetchall()
            else:
                rows = conn.execute(
    ...
    def add_message(
        ...
        with get_db() as conn:
            ...
            row = conn.execute(
                "SELECT id, title FROM conversations WHERE id = %s", (conv_id,)
            ).fetchone()
            if not row:
                return None
    ...
    def update_report(self, report_id: int, **kwargs) -> bool:
        ...
        with get_db() as conn:
            # Increment version
            conn.execute(
                "UPDATE reports SET version = version + 1 WHERE id = %s",
                (report_id,)
            )

            cursor = conn.execute(
                f"UPDATE reports SET {set_clause} WHERE id = %s",
                values
            )
            return cursor.rowcount > 0
    ...
    def get_all_tags(self, tag_type: Optional[str] = None) -> list[Tag]:
        ...
        with get_db() as conn:
            if tag_type:
                rows = conn.execute(
                    "SELECT * FROM tags WHERE type = %s ORDER BY label",
                    (tag_type,)
                ).fetchall()
            else:
                rows = conn.execute(
    ...
    def get_tag_by_name(self, name: str) -> Optional[Tag]:
        ...
        with get_db() as conn:
            row = conn.execute(
                "SELECT * FROM tags WHERE name = %s", (name,)
            ).fetchone()
            if row:
    ...
    def set_report_tags(self, report_id: int, tag_names: list[str], update_timestamp: bool = True) -> bool:
        ...
        with get_db() as conn:
            ...
            if update_timestamp:
                conn.execute(
                    "UPDATE reports SET updated_at = %s WHERE id = %s",
                    (datetime.now().isoformat(), report_id)
                )
            return True
    ...
    def get_uploaded_file(self, file_id: int) -> Optional[UploadedFile]:
        ...
        with get_db() as conn:
            row = conn.execute(
                "SELECT * FROM uploaded_files WHERE id = %s", (file_id,)
            ).fetchone()
    ...
    def get_uploaded_file_by_hash(self, sha256_hash: str) -> Optional[UploadedFile]:
        ...
        with get_db() as conn:
            row = conn.execute(
                "SELECT * FROM uploaded_files WHERE sha256_hash = %s LIMIT 1", (sha256_hash,)
            ).fetchone()
    ...
    def delete_uploaded_file(self, file_id: int) -> bool:
        ...
        with get_db() as conn:
            cursor = conn.execute(
                "DELETE FROM uploaded_files WHERE id = %s", (file_id,)
            )
            return cursor.rowcount > 0
